...
class DuplexVoiceCallTestClient:
    ...
    async def test_duplex_voice_call(self, audio_file_path):
        """测试双向同时的语音电话功能"""
        ...
        try:
            # 发送语音请求
            await self.ws.send(json.dumps(voice_message))
            print("[测试] 语音请求已发送")
...
async def main():
    ...
    try:
        ...
        if os.path.exists(audio_file_path):
            print("\n=== 测试双向同时的语音电话功能 ===")
            await client.test_duplex_voice_call(audio_file_path)
        else:
            print(f"[测试] 音频文件不存在: {audio_file_path}")
