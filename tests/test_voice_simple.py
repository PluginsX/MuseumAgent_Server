...
class SimpleVoiceTestClient:
    ...
    async def send_voice_request(self, audio_file_path):
        """发送语音请求（使用REQUEST消息类型）"""
        ...
        request_message = {
            ...
            "payload": {
                ...
                "content": {
                    "voice_mode": "BASE64",
                    "voice": base64.b64encode(audio_data).decode('utf-8')
                }
            },
            "timestamp": int(time.time() * 1000)
        }
...
async def main():
    ...
    try:
        ...
        if os.path.exists(short_audio_path):
            print("\n=== 测试短语音 (BASE64模式) ===")
            await client.send_voice_request(short_audio_path)
        else:
        ...
        if os.path.exists(long_audio_path):
            print("\n=== 测试长语音 (BASE64模式) ===")
            await client.send_voice_request(long_audio_path)
        else:
