...
export default function ClientManagement() {
  ...
  const handleCreateClient = async (values: any) => {
    try {
      // 先创建客户
      const userResponse = await clientsApiExtended.createClient(values);
      const user = extractData<User>(userResponse);
      
      // 然后为客户生成API密钥
      const apiKeyResponse = await clientsApiExtended.resetApiKey(user.id);
      const apiKeyData = extractData<{ api_key: string }>(apiKeyResponse);
      ...
      Modal.success({
        ...
        content: (
          <div>
            ...
            <p>API密钥: <strong>{apiKeyData.api_key}</strong></p>
