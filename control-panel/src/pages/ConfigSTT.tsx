...
export default function ConfigSTT() {
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        console.log('Loading STT config...');
        console.log('Calling configApi.getSTT()...');
        const response = await configApi.getSTT();
  ...
  const onFinish = async (values: { config: string }) => {
    setLoading(true);
    try {
      console.log('Saving STT config:', values);
      ...
      await configApi.updateSTT(configData as any);
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      setTesting(true);
      console.log('Testing STT connection...');
      
      // 异步执行测试连接，不阻塞主线程
      const response = await configApi.validateSTT({
        base_url: configData.base_url as string,
        api_key: configData.api_key as string,
        model: configData.model as string,
      });
      
      console.log('STT validation response:', response);
      ...
      console.log('STT validation result:', { isValid, messageText });
      ...
      if (e instanceof SyntaxError) {
        ...
        console.error('STT validation error:', e);
