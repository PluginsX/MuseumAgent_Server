...
- `stream_seq`: `0`
- `content.text`: 空字符串 `""`
- 通过 `require_tts` 和/或 `function_calling_op` + `function_calling` 更新会话
...
2. 服务端先处理会话属性更新（`require_tts`、`function_calling`）
3. 文本：直接使用；语音：流式转发 STT，等待完整文本（关闭中间结果）
4. 用完整文本调用 SRS 做语义检索
5. 构建提示词：用户输入 + 函数定义 + SRS 结果，调用 LLM
...
- REQUEST 中若提供 `require_tts` 或 `function_calling_op` + `function_calling`，立即覆盖会话缓存
- 语音请求：STT 流式输入、完整输出；仅在拿到完整文本后调用 SRS 与 LLM
